...
const passwordCheck = (validationObj) => {
    // Password requirements were not defined - Regex below allows capitalized letters, numbers and some special characters (no minimum requirement), with a minimum length of 8
    let passwordRegex = new RegExp("^[A-Za-z0-9@$!%*?&]{8,}$")

    // Added 'undefined' check since the first check was running the regex test on 'undefined' as a string and letting it pass, sending through the object without a password.
