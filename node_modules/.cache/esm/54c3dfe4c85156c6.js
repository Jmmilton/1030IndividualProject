_c85‍.x([["validationCheck",()=>validationCheck],["missingEntryCheck",()=>missingEntryCheck]]);// Below is a list of individual field validation functions that are used in the main validation function - more can be added below and then called in the main function "validationCheck" if additional fields have to be validated (e.g. first/last names, address, etc.)
...
// Below is the propertyIncorrectCheck function, which can validate any number of properties in the list above - simply add in a check in this function to see if the property is present, and call the relevant function above with the validation rules
       ...
       const validationCheck = (entry, requiredFields) => {
...
// Below is a function that can check for a missing entry based on a searchValue property string (e.g. "email", "id", or "name") and a checkValue (e.g. req.body.email)
       ...
       const missingEntryCheck = (array, searchValue, checkValue) => {
